...
def load_bibliography(apps, schema_editor):
    ...
    p_cache,created=PersonCache.objects.get_or_create(
        long_name = str(aavv_fc.long_format.pattern),
        short_name = str(aavv_fc.short_format.pattern),
        list_name = str(aavv_fc.list_format.pattern),
        ordering_name = str(aavv_fc.ordering_format.pattern)
    )
    # p_cache=PersonCache()
