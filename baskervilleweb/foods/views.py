...
def diary_dated_items(uid,date_list, object_list, extra_context):
    ...
    if object_list:
        ...
        extra_context["ref_weight"]=models.WeightDiaryEntry.objects.filter(user=extra_context["userobj"]).filter(time__lte=date_ref).last()
    else:
        extra_context["ref_weight"]=models.WeightDiaryEntry.objects.filter(user=extra_context["userobj"]).last()
...
class AddDiariesView(View):
    formset_class = forms.DiaryFormset
    ...
    def recipe_parse(self,cleaned_data): 
        ...
        g=models.MeasureUnit.objects.get(name="g")
        ret=[]
    ...
    def post(self,request,*args,**kwargs):
        ...
        for form in formset:
            ...
            for product,quantity,measure in entry_list:
                models.FoodDiaryEntry.objects.create(user=user,time=d_time,
                                                     product=product,quantity=quantity,measure_unit=measure)
